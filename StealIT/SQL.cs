            ...
            try
            {
                if ((int)this._fileBytes[offset] == 13)
                {
                    ...
                    if (this._tableEntries != null)
                    {
                        ...
                        Array.Resize<TableEntry>(ref this._tableEntries, this._tableEntries.Length + (int)num1 + 1);
                    }
                    else
                        this._tableEntries = new TableEntry[(int)num1 + 1];
                    for (ushort index1 = 0; (int)index1 <= (int)num1; ++index1)
                    {
                        ...
                        RecordHeaderField[] array = (RecordHeaderField[])null;
                        long num6 = (long)num4 - (long)endIdx3 + 1L;
                        int index2 = 0;
                        while (num6 < num5)
                        {
                            Array.Resize<RecordHeaderField>(ref array, index2 + 1);
                            int startIdx = endIdx4 + 1;
                            endIdx4 = this.Gvl(startIdx);
                            array[index2].Type = this.Cvl(startIdx, endIdx4);
                            array[index2].Size = array[index2].Type <= 9L ? (long)this._sqlDataTypeSize[array[index2].Type] : (!IsOdd(array[index2].Type) ? (array[index2].Type - 12L) / 2L : (array[index2].Type - 13L) / 2L);
                ...
                switch (this._fileBytes[offset])
                {
                    ...
                    case 13:
                        ...
                        if (this._masterTableEntries != null)
                        {
                            ...
                            this._masterTableEntries = new SqliteMasterEntry[checked((ulong)unchecked((long)num3 + 1L))];
